...
const CartPageItem = ({ ...element }) => {
  const dispatch = useDispatch();
  const onClickRemoveItem = () => {
    dispatch(removeItemFromCart(element.book.id));
  };
    ...
    <div className="content__items">
      <div className="cart__item">
        <div className="cart__item-img">
          <img className="bookImg" src={element.book.photo} alt="" />
        </div>
        <div className="cart__item-info">
          <h3>{element.book.title}</h3>
          <p>{element.book.author}</p>
        </div>
        <div className="cart__item-price">
          <b>{element.book.price} грн</b>
        </div>
        <div className="cart__item-remove">
          <div>
            <svg
              onClick={onClickRemoveItem}
              className="revomeItembutton"
              width="10"
              height="10"
              viewBox="0 0 10 10"
              fill="none"
